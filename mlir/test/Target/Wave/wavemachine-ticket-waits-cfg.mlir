...
func.func @cfg_join_nonzero(%cond: i1, %x: !wavemachine.reg<1, 1>) {
  ...
  %a = wavemachine.s_load_b32 %zero, "s[0:1]" : (!wavemachine.imm) -> !wavemachine.reg<0, 1>
  cf.cond_br %cond, ^then, ^else
^then:
  %b = wavemachine.s_load_b32 %zero, "s[0:1]" : (!wavemachine.imm) -> !wavemachine.reg<0, 1>
...
func.func @structured_for_double_buffer(%x: !wavemachine.reg<1, 1>) {
  %zero = wavemachine.imm 0 : !wavemachine.imm
  %init = wavemachine.s_load_b32 %zero, "s[0:1]" : (!wavemachine.imm) -> !wavemachine.reg<0, 1>
  %c0 = arith.constant 0 : index
  %c4 = arith.constant 4 : index
  %c1 = arith.constant 1 : index
  %res = scf.for %i = %c0 to %c4 step %c1 iter_args(%cur = %init) -> (!wavemachine.reg<0, 1>) {
    %next = wavemachine.s_load_b32 %zero, "s[0:1]" : (!wavemachine.imm) -> !wavemachine.reg<0, 1>
...
func.func @structured_for_triple_buffer(%x: !wavemachine.reg<1, 1>) {
  %zero = wavemachine.imm 0 : !wavemachine.imm
  %init0 = wavemachine.s_load_b32 %zero, "s[0:1]" : (!wavemachine.imm) -> !wavemachine.reg<0, 1>
  %init1 = wavemachine.s_load_b32 %zero, "s[0:1]" : (!wavemachine.imm) -> !wavemachine.reg<0, 1>
  %c0 = arith.constant 0 : index
  %c4 = arith.constant 4 : index
  %c1 = arith.constant 1 : index
  %r0, %r1 = scf.for %i = %c0 to %c4 step %c1
    ...
    %future0 = wavemachine.s_load_b32 %zero, "s[0:1]" : (!wavemachine.imm) -> !wavemachine.reg<0, 1>
    %sum = wavemachine.v_add_u32 %x, %cur : (!wavemachine.reg<1, 1>, !wavemachine.reg<0, 1>) -> !wavemachine.reg<1, 1>
